...
def by_column_video(params, title):
...
if __name__ == "__main__":
    ...
    archive = open(sys.argv[6])

    elites = archive.readlines()

    if sys.argv[5] == "by_column":
        for elite in elites:
            ...
            title = io_str_list[0]
            by_column_video(params, title)
    elif sys.argv[5] == "by_voxel":
        for elite in elites:
            io_str_list = elite.split(" ")
            params = []
            for i in range(5, len(io_str_list) - 1):
